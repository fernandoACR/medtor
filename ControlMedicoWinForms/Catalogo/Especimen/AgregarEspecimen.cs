...
{
    public partial class AgregarEspecimen : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        ControlEspecimen padre;
        CatalogoServices services = new CatalogoServices();

        public AgregarEspecimen(ControlEspecimen p)
        {
            padre = p;
            InitializeComponent();
        }
        ...
        private async void btnAgregar_Click(object sender, EventArgs e)
        {
            if (txtDescripcion.Text != "")
            {
                try
                {
                    if (await services.AddEspecimen(new EspecimenModel() { Descripcion = txtDescripcion.Text, Medico = LoginModel.Medico }) > 0)
                    {
                        ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show(ControlLenguaje.cadenas.GetString("EspecimenAgregado"), ControlLenguaje.cadenas.GetString("NombreAplicacion"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                        padre.Cargar();
                        this.Close();
                    }
                }
        ...
        private void AgregarEspecimen_Load(object sender, EventArgs e)
        {
